...
import type { Schema } from '../amplify/modules/todos/resource';
...
function App() {
	...
	const fetchTodos = async () => {
		...
		if (data?.statusCode === '200') setTodos(data.todoList!);
		else alert('Failed to load Todos');
	};
	...
	const createTodo = async () => {
		...
		if (res.data?.statusCode != '200') alert('Failed to create todo');
	...
	const updateTodo = async (todo: Todo) => {
		...
		if (res.data?.statusCode != '200') alert('Failed to update todo');
	...
	async function deleteTodo(
		...
		if (res.data?.statusCode != '200') alert('Failed to delete todo');
